-- ─────────────────────────────────────────────────────────────────────────────
-- Remove winning-clan CP head start from monthly_reset()
--
-- Previously step h awarded 50 CP to every member of the winning clan after
-- the reset, so they began the new month ahead of everyone else.
-- This migration removes that step so ALL students start the new month at
-- exactly 0 CP regardless of their previous clan's performance.
--
...
begin
  ...
  -- f) reset all CP to 0 ----------------------------------------
  -- All students — including winning-clan members — start the new
  -- month at exactly 0 CP. No head-start advantage is applied.
  ...
  -- Step h previously set cp = 50 for winning-clan members and
  -- inserted clan_winner_headstart awards. Removed so every student
  -- starts the new month at 0 CP.

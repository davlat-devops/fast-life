...
--
-- Fixes: "DELETE requires a WHERE clause" error thrown when monthly_reset()
-- runs DELETE FROM cp_awards / DELETE FROM cp_deductions without a WHERE.
-- Supabase rejects unconditional DELETEs; WHERE true makes the intent explicit.
--
-- This supersedes cascade_deductions_and_monthly_reset_v3.sql — apply this
-- instead of (or after) that file. Safe to run on a live database.
-- ─────────────────────────────────────────────────────────────────────────────
...
begin
  ...
  -- WHERE true makes the unconditional intent explicit so Supabase
  -- does not reject these as accidentally unscoped DELETEs.
